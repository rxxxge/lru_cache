...
int main(void) {
    ...
    if (remove_hash_entry("unknown_key", hash_table, RESIZE_AUTOMATICALLY) == SUCCESS) {
        fprintf(stderr, "TEST 5 FAILED: Removed key that doesn't exist!\n");
        exit(EXIT_FAILURE);
    }

    if (remove_hash_entry("key3", hash_table, RESIZE_AUTOMATICALLY) != SUCCESS) {
        fprintf(stderr, "TEST 6 FAILED: Did not remove existing key!\n");
        exit(EXIT_FAILURE);
    }
